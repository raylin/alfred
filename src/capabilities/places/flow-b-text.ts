...
export async function runFlowB(
  ...
  // 7. Send Flex Message reply (with disambiguation note if multiple candidates)
  ...
  await sendReply(replyToken, [buildDraftCard(fullPlace, note)], env.LINE_CHANNEL_ACCESS_TOKEN, chatId)
}
